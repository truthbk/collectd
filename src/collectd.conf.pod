...
=head2 Plugin C<write_datadog>

This output plugin submits values to Datadog via the API using POST requests and
encoding metrics in JSON as described in their documentation:
L<http://docs.datadoghq.com/api/>. Currently this plugin will only submit SNMP
metrics.

Synopsis:

 <Plugin "write_datadog">
   <Node "example">
     Api-Key "somedatadogapikey"
   </Node>
 </Plugin>

The plugin can send values for multiple Datadog accounts, should you have them, use a
E<lt>B<Node>E<nbsp>I<Name>E<gt> block for each API key. Within each B<Node>
block, the following options are available:

=over 4

=item B<Api-Key> I<String>

Mandatory API-Key required for authentication.

=item B<Tags> I<String>

Optional set of tags you'd like to apply to the values sent to Datadog.
...
The C<write_datadog> plugin regularly submits the collected values to the Datadog
API endpoint. How frequently this happens depends on how much data you are
collecting and the size of B<BufferSize>. The optimal value to set B<Timeout>
to is slightly below this interval, which you can estimate by monitoring the
